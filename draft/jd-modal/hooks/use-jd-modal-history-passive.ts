...
export const useJdModalHistoryPassive = () => {
  const modalService = useJdModalService();
  let usedHistory = modalService.enabledHistoryStrategy;
  let modalId: number | null = null;
  let scription: Subscription | null = null;
   ...
   */
  const openBefore = () => {
    usedHistory = modalService.enabledHistoryStrategy;
    modalService.setEnableHistoryStrategy(false);
  };
  ...
  const onClosed = () => {
    disposeScription();
    modalService.setEnableHistoryStrategy(usedHistory);
  ...
  const onPopstate = () => {
    disposeScription();
    const modalRef = modalService.getModalRef(modalId as number);
    if (modalRef) {
      modalRef.close();
    }
    modalId = null;
